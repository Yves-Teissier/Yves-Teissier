...
THEORY ListVisibleVariablesX IS
  ...
  List_VisibleVariables(Implementation(ControlUnit2_i))==(IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_cu2_main_state,prev_cu1_main_state,prev_main_state,cu2_main_state,cu1_main_state,cu2_error,current_u);
  Internal_List_VisibleVariables(Implementation(ControlUnit2_i))==(IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_cu2_main_state,prev_cu1_main_state,prev_main_state,cu2_main_state,cu1_main_state,cu2_error,current_u)
END
&
THEORY ListInvariantX IS
  ...
  List_Invariant(Implementation(ControlUnit2_i))==(current_u: NAT & cu2_error: BOOL & cu1_main_state: MAIN_STATE & cu2_main_state: MAIN_STATE & prev_cu1_main_state: MAIN_STATE & prev_cu2_main_state: MAIN_STATE & prev_main_state: MAIN_STATE & IS_FIRST_ELECTION: BOOL & IS_MASTER_BEFORE: BOOL & IS_ERROR_BEFORE: BOOL & IS_ERROR_RECEIVED_BEFORE: BOOL & IS_MASTER: BOOL)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(ControlUnit2_i))==((0: INT | current_u:=0);cu2_error:=FALSE;cu1_main_state:=OFF;cu2_main_state:=OFF;prev_cu1_main_state:=OFF;prev_cu2_main_state:=OFF;prev_main_state:=OFF;IS_FIRST_ELECTION:=TRUE;IS_MASTER_BEFORE:=FALSE;IS_ERROR_BEFORE:=FALSE;IS_ERROR_RECEIVED_BEFORE:=FALSE;IS_MASTER:=FALSE);
  Context_List_Initialisation(Implementation(ControlUnit2_i))==(skip);
  List_Initialisation(Implementation(ControlUnit2_i))==(current_u:=0;cu2_error:=FALSE;cu1_main_state:=OFF;cu2_main_state:=OFF;prev_cu1_main_state:=OFF;prev_cu2_main_state:=OFF;prev_main_state:=OFF;IS_FIRST_ELECTION:=TRUE;IS_MASTER_BEFORE:=FALSE;IS_ERROR_BEFORE:=FALSE;IS_ERROR_RECEIVED_BEFORE:=FALSE;IS_MASTER:=FALSE)
END
&
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_next_running_state)==(state: RUNNING_STATE | state = MASTER ==> IS_MASTER_BEFORE:=TRUE [] not(state = MASTER) ==> IS_MASTER_BEFORE:=FALSE;(state = SLAVE ==> (SLAVE: RUNNING_STATE & 2: NAT | skip) [] not(state = SLAVE) ==> (state = MASTER ==> (MASTER: RUNNING_STATE & 2: NAT | skip) [] not(state = MASTER) ==> skip));(CU2_IS_FIRST_SLAVE = TRUE & IS_FIRST_ELECTION = TRUE ==> (next_running_state:=SLAVE;(SLAVE: RUNNING_STATE & 2: NAT | skip)) [] not(CU2_IS_FIRST_SLAVE = TRUE & IS_FIRST_ELECTION = TRUE) ==> skip);(CU2_IS_FIRST_SLAVE = FALSE & IS_FIRST_ELECTION = TRUE ==> (next_running_state:=MASTER;(MASTER: RUNNING_STATE & 2: NAT | skip)) [] not(CU2_IS_FIRST_SLAVE = FALSE & IS_FIRST_ELECTION = TRUE) ==> skip);(IS_MASTER_BEFORE = TRUE & IS_ERROR_BEFORE = TRUE ==> (next_running_state:=SLAVE;(SLAVE: RUNNING_STATE & 2: NAT | skip)) [] not(IS_MASTER_BEFORE = TRUE & IS_ERROR_BEFORE = TRUE) ==> skip);(IS_MASTER_BEFORE = FALSE & IS_ERROR_RECEIVED_BEFORE = TRUE ==> (next_running_state:=MASTER;(MASTER: RUNNING_STATE & 2: NAT | skip)) [] not(IS_MASTER_BEFORE = FALSE & IS_ERROR_RECEIVED_BEFORE = TRUE) ==> skip);(next_running_state = UNKNOWN ==> (UNKNOWN: RUNNING_STATE & 2: NAT | skip) [] not(next_running_state = UNKNOWN) ==> skip);IS_FIRST_ELECTION:=FALSE);
  ...
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_next_main_state)==(state: MAIN_STATE | (0: NAT & 1: NAT | @(resp_cu_main_state$0).(resp_cu_main_state$0: MAIN_STATE ==> prev_cu1_main_state:=resp_cu_main_state$0));(prev_cu1_main_state = ERROR ==> IS_ERROR_RECEIVED_BEFORE:=TRUE [] not(prev_cu1_main_state = ERROR) ==> IS_ERROR_RECEIVED_BEFORE:=FALSE);(0: NAT & 2: NAT | @(resp_cu_main_state$0).(resp_cu_main_state$0: MAIN_STATE ==> prev_cu2_main_state:=resp_cu_main_state$0));(prev_cu2_main_state = ERROR ==> IS_ERROR_BEFORE:=TRUE [] not(prev_cu2_main_state = ERROR) ==> IS_ERROR_BEFORE:=FALSE);(state = ERROR ==> (ERROR: MAIN_STATE & 2: NAT | skip) [] not(state = ERROR) ==> (state = SHUTDOWN ==> (prev_main_state: MAIN_STATE & 2: NAT | skip) [] not(state = SHUTDOWN) ==> skip));(btrue | @(resp_cu2_error$0).(resp_cu2_error$0: BOOL ==> cu2_error:=resp_cu2_error$0));(cu2_error = TRUE ==> (next_main_state:=ERROR;(ERROR: MAIN_STATE & 2: NAT | skip)) [] not(cu2_error = TRUE) ==> ((btrue | @(resp_current_u$0).(resp_current_u$0: NAT ==> current_u:=resp_current_u$0));(current_u>32 ==> next_main_state:=ON [] not(current_u>32) ==> (current_u>32 & current_u<34 ==> next_main_state:=ON [] not(current_u>32 & current_u<34) ==> (current_u<30 or current_u>35 ==> next_main_state:=SHUTDOWN [] not(current_u<30 or current_u>35) ==> (current_u<32 ==> next_main_state:=OFF [] not(current_u<32) ==> skip))));(next_main_state: MAIN_STATE & 2: NAT | skip)));cu2_main_state:=next_main_state);
  List_Substitution(Implementation(ControlUnit2_i),cu2_next_main_state)==(prev_cu1_main_state <-- mb_get_cu_main_state(0,1);IF prev_cu1_main_state = ERROR THEN IS_ERROR_RECEIVED_BEFORE:=TRUE ELSE IS_ERROR_RECEIVED_BEFORE:=FALSE END;prev_cu2_main_state <-- mb_get_cu_main_state(0,2);IF prev_cu2_main_state = ERROR THEN IS_ERROR_BEFORE:=TRUE ELSE IS_ERROR_BEFORE:=FALSE END;IF state = ERROR THEN mb_set_main_state(ERROR,2) ELSIF state = SHUTDOWN THEN mb_set_main_state(prev_main_state,2) END;cu2_error <-- mb_getCU2Error;IF cu2_error = TRUE THEN next_main_state:=ERROR;mb_set_main_state(ERROR,2) ELSE current_u <-- mb_getCurrentU;IF current_u>32 THEN next_main_state:=ON ELSIF current_u>32 & current_u<34 THEN next_main_state:=ON ELSIF current_u<30 or current_u>35 THEN next_main_state:=SHUTDOWN ELSIF current_u<32 THEN next_main_state:=OFF END;mb_set_main_state(next_main_state,2) END;cu2_main_state:=next_main_state);
  ...
  List_Substitution(Implementation(ControlUnit2_i),cu2_next_running_state)==(IF state = MASTER THEN IS_MASTER_BEFORE:=TRUE ELSE IS_MASTER_BEFORE:=FALSE END;IF state = SLAVE THEN mb_set_running_state(SLAVE,2) ELSIF state = MASTER THEN mb_set_running_state(MASTER,2) END;IF CU2_IS_FIRST_SLAVE = TRUE & IS_FIRST_ELECTION = TRUE THEN next_running_state:=SLAVE;mb_set_running_state(SLAVE,2) END;IF CU2_IS_FIRST_SLAVE = FALSE & IS_FIRST_ELECTION = TRUE THEN next_running_state:=MASTER;mb_set_running_state(MASTER,2) END;IF IS_MASTER_BEFORE = TRUE & IS_ERROR_BEFORE = TRUE THEN next_running_state:=SLAVE;mb_set_running_state(SLAVE,2) END;IF IS_MASTER_BEFORE = FALSE & IS_ERROR_RECEIVED_BEFORE = TRUE THEN next_running_state:=MASTER;mb_set_running_state(MASTER,2) END;IF next_running_state = UNKNOWN THEN mb_set_running_state(UNKNOWN,2) END;IS_FIRST_ELECTION:=FALSE)
END
&
...
THEORY InheritedEnvX IS
  VisibleVariables(Implementation(ControlUnit2_i))==(Type(current_u) == Mvv(btype(INTEGER,?,?));Type(cu2_error) == Mvv(btype(BOOL,?,?));Type(cu1_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(cu2_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(prev_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(prev_cu1_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(prev_cu2_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(IS_FIRST_ELECTION) == Mvv(btype(BOOL,?,?));Type(IS_MASTER_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_RECEIVED_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_MASTER) == Mvv(btype(BOOL,?,?)));
...
THEORY ListOfIdsX IS
  ...
  List_Of_VisibleVar_Ids(Implementation(ControlUnit2_i)) == (IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_cu2_main_state,prev_cu1_main_state,prev_main_state,cu2_main_state,cu1_main_state,cu2_error,current_u | ?);
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Implementation(ControlUnit2_i)) == (Type(IS_MASTER) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_RECEIVED_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_MASTER_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_FIRST_ELECTION) == Mvv(btype(BOOL,?,?));Type(prev_cu2_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(prev_cu1_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(prev_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(cu2_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(cu1_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(cu2_error) == Mvv(btype(BOOL,?,?));Type(current_u) == Mvv(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
...
THEORY TypingPredicateX IS
  TypingPredicate(Implementation(ControlUnit2_i))==(current_u: INTEGER & cu2_error: BOOL & cu1_main_state: MAIN_STATE & cu2_main_state: MAIN_STATE & prev_main_state: MAIN_STATE & prev_cu1_main_state: MAIN_STATE & prev_cu2_main_state: MAIN_STATE & IS_FIRST_ELECTION: BOOL & IS_MASTER_BEFORE: BOOL & IS_ERROR_BEFORE: BOOL & IS_ERROR_RECEIVED_BEFORE: BOOL & IS_MASTER: BOOL)
END
&
