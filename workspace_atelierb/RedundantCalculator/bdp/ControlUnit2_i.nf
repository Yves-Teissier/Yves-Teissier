...
THEORY ListVisibleVariablesX IS
  ...
  List_VisibleVariables(Implementation(ControlUnit2_i))==(IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_main_state,cu2_error,current_u);
  Internal_List_VisibleVariables(Implementation(ControlUnit2_i))==(IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_main_state,cu2_error,current_u)
END
&
THEORY ListInvariantX IS
  ...
  List_Invariant(Implementation(ControlUnit2_i))==(current_u: NAT & cu2_error: BOOL & prev_main_state: MAIN_STATE & IS_FIRST_ELECTION: BOOL & IS_MASTER_BEFORE: BOOL & IS_ERROR_BEFORE: BOOL & IS_ERROR_RECEIVED_BEFORE: BOOL & IS_MASTER: BOOL)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(ControlUnit2_i))==((0: INT | current_u:=0);cu2_error:=FALSE;prev_main_state:=OFF;IS_FIRST_ELECTION:=TRUE;IS_MASTER_BEFORE:=FALSE;IS_ERROR_BEFORE:=FALSE;IS_ERROR_RECEIVED_BEFORE:=FALSE;IS_MASTER:=FALSE);
  Context_List_Initialisation(Implementation(ControlUnit2_i))==(skip);
  List_Initialisation(Implementation(ControlUnit2_i))==(current_u:=0;cu2_error:=FALSE;prev_main_state:=OFF;IS_FIRST_ELECTION:=TRUE;IS_MASTER_BEFORE:=FALSE;IS_ERROR_BEFORE:=FALSE;IS_ERROR_RECEIVED_BEFORE:=FALSE;IS_MASTER:=FALSE)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(ControlUnit2_i))==(cu2_next_main_state,cu2_next_on_state,cu2_next_running_state,cu2_run);
  List_Operations(Implementation(ControlUnit2_i))==(cu2_next_main_state,cu2_next_on_state,cu2_next_running_state,cu2_run)
END
&
THEORY ListInputX IS
  ...
  List_Input(Implementation(ControlUnit2_i),cu2_run)==(?)
END
&
THEORY ListOutputX IS
  ...
  List_Output(Implementation(ControlUnit2_i),cu2_run)==(?)
END
&
THEORY ListHeaderX IS
  ...
  List_Header(Implementation(ControlUnit2_i),cu2_run)==(cu2_run)
END
&
THEORY ListPreconditionX IS
  ...
  Own_Precondition(Implementation(ControlUnit2_i),cu2_run)==(btrue);
  List_Precondition(Implementation(ControlUnit2_i),cu2_run)==(btrue)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_run)==(btrue | skip);
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_next_running_state)==(state: RUNNING_STATE | state = SLAVE ==> (SLAVE: RUNNING_STATE & 2: NAT | skip) [] not(state = SLAVE) ==> (state = MASTER ==> (MASTER: RUNNING_STATE & 2: NAT | skip) [] not(state = MASTER) ==> skip));
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_next_on_state)==(state: ON_STATE | (btrue | @(current_u$0).(current_u$0: NAT ==> current_u:=current_u$0));(current_u>U_LOW & current_u<U_HIGH ==> next_on_state:=RUNNING [] not(current_u>U_LOW & current_u<U_HIGH) ==> skip);(state = INITIALIZING ==> (INITIALIZING: ON_STATE & 2: NAT | skip) [] not(state = INITIALIZING) ==> skip));
  Expanded_List_Substitution(Implementation(ControlUnit2_i),cu2_next_main_state)==(state: MAIN_STATE | state = ERROR ==> (ERROR: MAIN_STATE & 2: NAT | skip) [] not(state = ERROR) ==> (state = SHUTDOWN ==> (prev_main_state: MAIN_STATE & 1: NAT | skip) [] not(state = SHUTDOWN) ==> skip);(btrue | @(current_u$0).(current_u$0: NAT ==> current_u:=current_u$0));(current_u>U_MIN ==> next_main_state:=ON [] not(current_u>U_MIN) ==> skip);(current_u<U_UNDER or current_u>U_OVER ==> next_main_state:=SHUTDOWN [] not(current_u<U_UNDER or current_u>U_OVER) ==> skip);(current_u>U_LOW & current_u<U_HIGH ==> next_main_state:=ON [] not(current_u>U_LOW & current_u<U_HIGH) ==> skip);(current_u<U_MIN ==> next_main_state:=OFF [] not(current_u<U_MIN) ==> skip);(btrue | @(cu1_error$0).(cu1_error$0: BOOL ==> cu2_error:=cu1_error$0));(cu2_error = TRUE ==> next_main_state:=ERROR [] not(cu2_error = TRUE) ==> skip));
  List_Substitution(Implementation(ControlUnit2_i),cu2_next_main_state)==(IF state = ERROR THEN mb_set_main_state(ERROR,2) ELSIF state = SHUTDOWN THEN mb_set_main_state(prev_main_state,1) END;current_u <-- mb_getCurrentU;IF current_u>U_MIN THEN next_main_state:=ON END;IF current_u<U_UNDER or current_u>U_OVER THEN next_main_state:=SHUTDOWN END;IF current_u>U_LOW & current_u<U_HIGH THEN next_main_state:=ON END;IF current_u<U_MIN THEN next_main_state:=OFF END;cu2_error <-- mb_getCU1Error;IF cu2_error = TRUE THEN next_main_state:=ERROR END);
  List_Substitution(Implementation(ControlUnit2_i),cu2_next_on_state)==(current_u <-- mb_getCurrentU;IF current_u>U_LOW & current_u<U_HIGH THEN next_on_state:=RUNNING END;IF state = INITIALIZING THEN mb_set_on_state(INITIALIZING,2) END);
  List_Substitution(Implementation(ControlUnit2_i),cu2_next_running_state)==(IF state = SLAVE THEN mb_set_running_state(SLAVE,2) ELSIF state = MASTER THEN mb_set_running_state(MASTER,2) END);
  List_Substitution(Implementation(ControlUnit2_i),cu2_run)==(skip)
END
&
...
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(ControlUnit2_i),Machine(MailBox))==(mb_getCurrentU,mb_getCU1Error,mb_getCU2Error,mb_set_on_state,mb_set_running_state,mb_set_main_state)
END
&
THEORY InheritedEnvX IS
  VisibleVariables(Implementation(ControlUnit2_i))==(Type(current_u) == Mvv(btype(INTEGER,?,?));Type(cu2_error) == Mvv(btype(BOOL,?,?));Type(prev_main_state) == Mvv(etype(MAIN_STATE,?,?));Type(IS_FIRST_ELECTION) == Mvv(btype(BOOL,?,?));Type(IS_MASTER_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_ERROR_RECEIVED_BEFORE) == Mvv(btype(BOOL,?,?));Type(IS_MASTER) == Mvv(btype(BOOL,?,?)));
  Operations(Implementation(ControlUnit2_i))==(Type(cu2_run) == Cst(No_type,No_type);Type(cu2_next_running_state) == Cst(etype(RUNNING_STATE,?,?),etype(RUNNING_STATE,?,?));Type(cu2_next_on_state) == Cst(etype(ON_STATE,?,?),etype(ON_STATE,?,?));Type(cu2_next_main_state) == Cst(etype(MAIN_STATE,?,?),etype(MAIN_STATE,?,?)));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(ControlUnit2_i)) == (? | ? | ? | ? | cu2_next_main_state,cu2_next_on_state,cu2_next_running_state,cu2_run | ? | seen(Machine(CTX)),imported(Machine(MailBox)) | ? | ControlUnit2_i);
  List_Of_HiddenCst_Ids(Implementation(ControlUnit2_i)) == (? | ?);
  List_Of_VisibleCst_Ids(Implementation(ControlUnit2_i)) == (?);
  List_Of_VisibleVar_Ids(Implementation(ControlUnit2_i)) == (IS_MASTER,IS_ERROR_RECEIVED_BEFORE,IS_ERROR_BEFORE,IS_MASTER_BEFORE,IS_FIRST_ELECTION,prev_main_state,cu2_error,current_u | ?);
...
THEORY TypingPredicateX IS
  TypingPredicate(Implementation(ControlUnit2_i))==(current_u: INTEGER & cu2_error: BOOL & prev_main_state: MAIN_STATE & IS_FIRST_ELECTION: BOOL & IS_MASTER_BEFORE: BOOL & IS_ERROR_BEFORE: BOOL & IS_ERROR_RECEIVED_BEFORE: BOOL & IS_MASTER: BOOL)
END
&
