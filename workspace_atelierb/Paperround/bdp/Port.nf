...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Port))==(arrive,dock,leave,numberwaiting);
  List_Operations(Machine(Port))==(arrive,dock,leave,numberwaiting)
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Port)) == (SHIP,QUAY | ? | docked,waiting | ? | arrive,dock,leave,numberwaiting | ? | ? | ? | Port);
...
THEORY OperationsEnvX IS
  Operations(Machine(Port)) == (Type(numberwaiting) == Cst(btype(INTEGER,?,?),No_type);Type(leave) == Cst(atype(QUAY,?,?),atype(SHIP,?,?));Type(dock) == Cst(No_type,atype(QUAY,?,?));Type(arrive) == Cst(No_type,atype(SHIP,?,?)));
