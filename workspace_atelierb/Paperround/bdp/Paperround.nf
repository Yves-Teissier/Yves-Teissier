...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(Paperround))==(papers <: 1..163 & magazines <: papers & card(papers)<=60)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(Paperround))==(addpaper,addmagazine,remove);
  List_Operations(Machine(Paperround))==(addpaper,addmagazine,remove)
END
&
...
THEORY ListPreconditionX IS
  ...
  List_Precondition(Machine(Paperround),addmagazine)==(hh: papers);
  List_Precondition(Machine(Paperround),remove)==(hh: 1..163)
END
&
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(Paperround),remove)==(hh: 1..163 | magazines,papers:=magazines-{hh},papers-{hh});
  Expanded_List_Substitution(Machine(Paperround),addmagazine)==(hh: papers | magazines:=magazines\/{hh});
  ...
  List_Substitution(Machine(Paperround),remove)==(magazines:=magazines-{hh} || papers:=papers-{hh})
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Machine(Paperround)) == (? | ? | magazines,papers | ? | addpaper,addmagazine,remove | ? | ? | ? | Paperround);
...
THEORY OperationsEnvX IS
  Operations(Machine(Paperround)) == (Type(remove) == Cst(No_type,btype(INTEGER,?,?));Type(addmagazine) == Cst(No_type,btype(INTEGER,?,?));Type(addpaper) == Cst(No_type,btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
