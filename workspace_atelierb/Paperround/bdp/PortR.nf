...
THEORY ListOperationsX IS
  Internal_List_Operations(Refinement(PortR))==(arrive,dock,leave,numberwaiting);
  List_Operations(Refinement(PortR))==(arrive,dock,leave,numberwaiting)
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Refinement(PortR)) == (? | ? | num | fun,list | arrive,dock,leave,numberwaiting | ? | included(Machine(List)),included(Machine(Map)) | ? | PortR);
...
THEORY OperationsEnvX IS
  Operations(Refinement(PortR)) == (Type(numberwaiting) == Cst(btype(INTEGER,?,?),No_type);Type(leave) == Cst(atype(QUAY,?,?),atype(SHIP,?,?));Type(dock) == Cst(No_type,atype(QUAY,?,?));Type(arrive) == Cst(No_type,atype(SHIP,?,?)))
END
&
THEORY TCIntRdX IS
  predB0 == OK;
